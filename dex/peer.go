...
func (ps *peerSet) BuildNotaryConn(round uint64) {
	...
	for chainID := uint32(0); chainID < ps.gov.GetChainNum(round); chainID++ {
		...
		if _, ok := s[selfID]; !ok {
			...
			ps.srvr.AddGroup(notarySetName(chainID, round), nodes, 3)
			continue
		}
...
func (ps *peerSet) newNode(id string) *discover.Node {
	// TODO: use nodeTable to build node
	return &discover.Node{ID: discover.MustHexID(id)}
}
