				...
				if d.mode == FastSync || d.mode == LightSync {
					...
					for _, header := range chunk {
						if header.GovState != nil {
							log.Debug("#gov store state", "round", header.Round)
...
func (d *Downloader) processFastSyncContent(latest *types.Header) error {
	...
	if height := latest.Number.Uint64(); height > uint64(fsMinFullBlocks) {
		log.Debug("#pivot start1", "height", pivot)
		pivot = height - uint64(fsMinFullBlocks)
		log.Debug("#pivot start2", "height", pivot)
	}
	...
	for {
		...
		if atomic.LoadInt32(&d.committed) == 0 {
			...
			if height := latest.Number.Uint64(); height > pivot+2*uint64(fsMinFullBlocks) {
				...
				log.Debug("#pivot stale1", "height", pivot)
				pivot = height - uint64(fsMinFullBlocks)
				log.Debug("#pivot stale2", "height", pivot)
			}
		}
		log.Debug("#pivot", "height", pivot)
