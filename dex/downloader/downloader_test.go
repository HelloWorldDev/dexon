...
func (dl *downloadTester) Genesis() *types.Block {
	return dl.genesis
}
...
// InsertHeaderChain2 injects a new batch of headers into the simulated chain.
func (dl *downloadTester) InsertHeaderChain2(headers []*types.HeaderWithGovState, verifierCache *dexCore.TSigVerifierCache) (i int, err error) {
	dl.lock.Lock()
	defer dl.lock.Unlock()

	// Do a quick check, as the blockchain.InsertHeaderChain2 doesn't insert anything in case of errors
...
// InsertChain2 injects a new batch of blocks into the simulated chain.
func (dl *downloadTester) InsertChain2(blocks types.Blocks) (i int, err error) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
