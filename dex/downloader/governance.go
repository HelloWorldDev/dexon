...
func newGovernance(headState *types.GovState) *governance {
	...
	if statedb == nil || err != nil {
		log.Debug("##new governance", "statedb == nil", statedb == nil, "err != nil", err != nil)
		if err != nil {
			log.Debug("##new governance", "err != nil", err)
		}
	}

	st := statedb.StorageTrie(vm.GovernanceContractAddress)
...
func (g *governance) StoreState(s *types.GovState) {
	g.mu.Lock()
	defer g.mu.Unlock()
	log.Debug("#gov store state", "number", s.Number.Uint64())
	...
	if s.Number.Uint64() > g.headHeight {
		log.Debug("#gov store state change head root", "number", s.Number.Uint64())
		g.headRoot = s.Root
		g.headHeight = s.Number.Uint64()
		h := g.getHeadHelper()
		num := int64(h.LenRoundHeight().Uint64())
		for i := int64(0); i < num; i++ {
			hh := h.RoundHeight(big.NewInt(i)).Uint64()
			log.Debug("changed", "round", i, "height", hh)
		}
	}
}
...
func (g *governance) IsDKGFinal(round uint64) bool {
	...
	log.Debug("#gov IsDKGFinal", "round", round, "threshold", threshold, "count", count, "is final", count >= threshold)
