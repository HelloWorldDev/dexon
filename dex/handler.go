...
type ProtocolManager struct {
	...
	notarySetCh  chan core.NewNotarySetEvent
	notarySetSub event.Subscription
	roundCh      chan core.NextRoundEvent
...
func (pm *ProtocolManager) Start(srvr p2pServer, maxPeers int) {
	...
	/*
		pm.roundCh = make(chan core.NextRoundEvent)
		pm.roundSub = pm.gov.SubscribeNextRoundEvent(pm.roundCh)
		go pm.notarySetLoop()
	*/
...
func (pm *ProtocolManager) notarySetLoop() {
	...
	*/
	...
	// subscribe new node info event

	for {
		select {
		case event := <-pm.roundCh:
			pm.peers.BuildNotaryConn(event.Round + 1)
			pm.peers.BuildDKGConn(event.Round + 1)
			pm.peers.ForgetNotaryConn(event.Round - 1)
			pm.peers.ForgetDKGConn(event.Round)

			/*
				case meta := <-pm.nodemetaCh:
					pm.peers.NotifyNewNodeMeta(meta)
			*/
