...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == GetBlockHeadersMsg:
		...
		log.Debug("#r handler", "id", p.id[:4], "number", query.Origin.Number,
			"amount", query.Amount, "skip", query.Skip, "reverse", query.Reverse,
			"withgov", query.WithGov)
		...
		if query.WithGov {
			...
			for r, height := range round {
				log.Debug("#r include round", "round", r)
				h, err := pm.gov.GetRoundHeight(ctx, r)
				if err != nil {
					// TODO(sonic): if can not provide gov state
					// remove all following headers.
					panic(err)
				}
				log.Debug("#r snapshot height", "height", h)
				if h == 0 {
					h = height
				}
			...
			for _, header := range headers {
				...
				log.Debug("#r send header", "round", header.Round, "number", header.Number.Uint64(), "state == nil", header.GovState == nil)
			}
		}
	...
	case msg.Code == BlockHeadersMsg:
		...
		for _, header := range headers {
			log.Debug("#r receive header", "round", header.Round, "number", header.Number.Uint64(), "state == nil", header.GovState == nil)
		}
